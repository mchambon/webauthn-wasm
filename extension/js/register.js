...
navigator.credentials.create = async (options) => {
    ...
    try {
        ...
        const result = await callRegisterFromAuthInRegister(ctap2Data);
        console.log("CTAP2: Réponse du serveur reçue:", JSON.stringify(result, null, 2));




        // Vérifier que result contient les propriétés attendues
        //if (!result || !result.credentialId || !result.attestationObject || !result.clientDataJSON) {
        //    throw new Error("Réponse invalide du serveur: données manquantes");
        //}


        const rawId = safeBase64ToArrayBuffer(result.credentialId);
        ...
        const credential = {
            ...
            rawId: rawId,
            response: {
                attestationObject: safeBase64ToArrayBuffer(result.attestationObject),
                clientDataJSON: safeBase64ToArrayBuffer(result.clientDataJSON),
            },
            type: "public-key",
...
navigator.credentials.get = async (options) => {
    console.log("WebAuthn: Intercepted credentials.get call", options);
    const ctap2Data = {
        type: "webauthn.get",
        clientDataJSON: {
            type: "webauthn.get",
    ...
    console.log("CTAP2: Envoi de données pour get assertion:", ctap2Data);
    
    // Pour l'instant, retourne null comme dans votre code original
    // Si vous implémentez cette partie plus tard, vous utiliserez probablement une fonction
    // similaire à callRegisterFromAuthInRegister mais pour l'authentification
    return null;
};
